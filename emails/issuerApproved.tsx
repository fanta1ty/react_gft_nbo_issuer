...
export default function IssuerApproved() {
  const name = "{{.Name}}";
  const helpCenterUrl = "{{.HelpUrl}}";
  const callback = "{{.Callback}}";

  return (
    <Wrapper previewText={"Your Issuer Application has been approved!"}>
      <Container className="w-full max-w-[632px] px-4 mb-20">
        ...
        >
          Your Issuer Application has been approved!
        </Heading>
        <Paragraph className="font-semibold">Hello {name}</Paragraph>
        <Paragraph>
          We are pleased to inform you that your application has been approved.
          You can now use all issuer services. Please proceed to log in to your
        ...
        <Paragraph>
          Issue and manage digital certificates for your students from one
          single place
        </Paragraph>
        <Paragraph>
